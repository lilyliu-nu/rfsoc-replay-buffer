...
│
├── ip_repo/
│   └── replay_buf_top_1_0/          ← Vivado packaged IP
│       ├── component.xml
│       ├── hdl/
│       │   ├── replay_buf_top.vhd
│       │   ├── replay_ctrl_axi.vhd
│       │   ├── replay_ctrl_bram.vhd
│       │   └── cdc_sync.v
│       └── xgui/
│           └── replay_buf_top_v1_0.tcl
│
├── boards/
│   └── RFSoC4x2/
│       ├── integrate_replay_buf_mep.tcl   ← block design patch for MEP overlay
│       └── bitstream/                     ← (gitignored, generated by Vivado)
│           ├── mep_replay.bit
│           └── mep_replay.hwh
│
├── notebooks/
│   ├── 01_strath_sdr_test.ipynb     ← test on strath-sdr overlay
│   ├── 02_mep_sdr_test.ipynb        ← test on MEP overlay
│   └── waveforms/
│       └── README.md                ← instructions for placing .bin files
│
├── driver/
│   ├── replay_buffer.py             ← PYNQ driver class
│   └── ddr4_repeater.py             ← software repeat for large files
│
└── docs/
    ├── USAGE.md                     ← register map and usage guide
    └── sg_dma_extension.md          ← guide for implementing SG DMA
```
